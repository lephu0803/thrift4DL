...
class TResult(object):
        ...
        while True:
            ...
            if fid == 1:
                ...
                if ftype == TType.STRING:
                    self.error_message = iprot.readString().decode(
                        'utf-8') if sys.version_info[0] == 2 else iprot.readString()
                else:
                    iprot.skip(ftype)
            elif fid == 3:
                if ftype == TType.STRING:
                    self.response = iprot.readString().decode(
                        'utf-8') if sys.version_info[0] == 2 else iprot.readString()
                else:
                    iprot.skip(ftype)
            else:
    ...
    def write(self, oprot):
        ...
        if self.error_message is not None:
            ...
            oprot.writeString(self.error_message.encode(
                'utf-8') if sys.version_info[0] == 2 else self.error_message)
            oprot.writeFieldEnd()
        if self.response is not None:
            oprot.writeFieldBegin('response', TType.STRING, 3)
            oprot.writeString(self.response.encode('utf-8')
                              if sys.version_info[0] == 2 else self.response)
            oprot.writeFieldEnd()
